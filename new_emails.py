...
v_last_cycle_first_day = sys_arg[0]  #the first day of previous Shamsi month
PARAM_DATA_DAY = sys_arg[1]
param_master = sys_arg[2]
PARAM_HDFS_ADDR = sys_arg[3]

spark = SparkSession.builder.appName('new_emails_yyyymm_cnt').master(param_master).getOrCreate()
...
tbl_bill_sent_mail_all = join2.select(col("acct_code").alias("ACCT_CODE"), col("email").alias("EMAIL"), col("msisdn").alias("MSISDN"))\
    .where((col("exp_date") > expr("to_date('" + PARAM_DATA_DAY + "','yyyyMMdd')")) &\
          (col("eff_date") >= expr("to_date('" + v_last_cycle_first_day + "','yyyyMMdd')"))
          )
...
tbl_bill_sent_mail_all\
    ...
    .option("header", True)\
    .mode('overwrite')\
    .csv("hdfs://" + PARAM_HDFS_ADDR + "/user/bdi/CRM/RESULT_DATA/new_emails_yyyymm_cnt/" + PARAM_DATA_DAY)